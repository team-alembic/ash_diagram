defmodule AshDiagram.Data.PolicySimulationTest do
  ...
  describe "Structure and Composition" do
    ...
    test "creates proper clause connections" do
      diagram = PolicySimulation.for_resource(Org)
      ...
      # Should have edges connecting clauses
      clause_edges =
        Enum.filter(edges, fn edge ->
          String.starts_with?(edge.from, "clause_") or String.starts_with?(edge.to, "clause_")
        end)

      assert length(clause_edges) > 0
    end
    ...
    test "creates subgraphs for multiple clauses" do
      diagram = PolicySimulation.for_resource(Org)
      subgraphs = Enum.filter(diagram.entries, &match?(%Subgraph{}, &1))

      # Should have clause subgraphs for multiple clauses
      clause_subgraphs = Enum.filter(subgraphs, fn sg -> String.starts_with?(IO.iodata_to_binary(sg.id), "clause_") end)
      assert length(clause_subgraphs) > 0
    end
  ...
  describe "Visual Tests" do
    ...
    test "renders field-specific policy simulation", %{tmp_dir: tmp_dir} do
      ...
      diagram |> Flowchart.compose() |> IO.iodata_to_binary() |> IO.puts()
